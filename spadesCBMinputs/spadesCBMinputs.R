...
defineModule(sim, list(
  ...
  outputObjects = bind_rows(
    ...
    createsOutput(objectName = "gcHash", objectClass = "matrix",
                  desc = ""), # TODO: description needed
    createsOutput(objectName = "level3DT", objectClass = "data.table",
    ...
    createsOutput(objectName = "mySpuDmids", objectClass = "data.frame",
                  desc = "the table containing one line per pixel"),
    createsOutput(objectName = "disturbanceRasters", objectClass = "raster",
                  desc = "Character vector of the disturbance rasters for SK"),
    createsOutput(objectName = "masterRaster", objectClass = "raster",
...
Init <- function(sim) {
  ...
  ## Rasters----------------------------------------------------------------------
  age <- raster(file.path("data/forIan/SK_data/CBM_GIS/age_TestArea.tif")) ## TODO: use module data path
  # This works
  ages <- getValues(age)
  # read-in species
  ldSpsRaster <- raster(file.path("data/forIan/SK_data/CBM_GIS/ldSp_TestArea.tif")) ## TODO: use module data path
  rasterSps <- getValues(ldSpsRaster) # 5 0 3 4 6 7
  # read-in productivity  levels
  prodRaster <- raster(file.path("data/forIan/SK_data/CBM_GIS/prod_TestArea.tif")) ## TODO: use module data path
  Productivity <- getValues(prodRaster) # 1 2 3 0
  # read-in spatial units
  spuRaster <- raster(file.path("data/forIan/SK_data/CBM_GIS/spUnits_TestArea.tif")) ## TODO: use module data path
  ...
  spatialDT <- spatialDT[, .(
    ages, rasterSps, spatial_unit_id, pixelIndex,
  ...
  table6 <- read.csv("data/appendix2_table6_v2.csv") ## TODO: use module data path for each of these CSVs
  ...
  # read-in species match with canfi_species code and genus to get right
  # Boudewyn params---------------------------------------------------
  ## danger this is hard coded ## Species match will have to be checked by user
  spsMatch <- fread("data/spsMatchNameRasterGfileBiomParams.csv") # file.path(paths(sim)$inputPath,"spsMatchNameRasterGfileBiomParams.csv"  ## TODO: use module data path
  ...
  for (i in 1:length(fullSpecies)) {
    ...
    for (j in 1:length(unique(speciesMeta$growth_curve_component_id))) {
      ...
      cumBiom <- as.matrix(convertM3biom(
        meta = meta, gCvalues = growthCurveComponents, spsMatch = spsMatch,
  ...
  retInt <- merge(sim$level3DT[, ], sim$cbmData@spinupParameters[, c(1, 2)],
