...
#HERE ## need to create my level3DT
  ...
  #Ian's stuff
  rasterMatch1 <- postProcess(x = rasters$au, rasterToMatch = masterRaster, filename2 = "pixelAUs.tif")
  AUs <- raster::projectRaster(from = rasters$au, to = masterRaster, method = 'ngb', filename = "pixelAUs.tif")
  AUs <- raster::projectRaster(from = rasters$au, to = masterRaster, method = 'ngb', filename = "pixelAUs.tif")
  plot(rasters$au)
  ...
  #gcIDold <- fread("data/spadesGCurvesSK.csv")#fread(sim$gcurveFileName)## danger hard coded##
### RIA
