...
setindex(level2DT, LeadingSp, Productivity, spatial_unit_id, Age)
head(level2DT)
...
# dim(unique(level3DT))
# [1] 759   8
#Hey Celine step below has to be done with left-join or it automatically sorts values and we lose location, I think....
#callled forest because it has no NA
forest <- left_join(level2DT, gcID, by = c("LeadingSp", "Productivity", "spatial_unit_id"))

#This was Eliot's suggestion as an alternative to the LBMR method
forest$PixelGroupID <- as.numeric(as.factor(paste(forest$spatial_unit_id, forest$growth_curve_component_id, forest$Age)))

#to set back into raster, need Raster (record of what is NA), at least first time. 
masterRaster <- raster(spuRaster)
masterRaster[rasterSps != 0] <- forest$PixelGroupID[!is.na(forest$growth_curve_component_id)] #1347529 pixels
plot(masterRaster)

#It works. Example of linking data back. You can delete this. 
temp <- level3DT 
temp$RootsCarbon <- temp$Age / runif(length(temp$Age), 10, 15) #simulated data from the CBM value
PixelGroupID <- data.table("PixelGroupID" = getValues(masterRaster)) 
RootsCarbon <- left_join(PixelGroupID, temp, by = "PixelGroupID")  #Need to make this merge

RootsCarbonRas <- raster(masterRaster)
RootsCarbonRas <- setValues(RootsCarbonRas, RootsCarbon$RootsCarbon)
plot(RootsCarbonRas)
rm(RootsCarbonRas, RootsCarbon, temp, PixelGroupID)
########################### DATA.TABLE COMPLETE #########################################
```