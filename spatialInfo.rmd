...
## Ian: is the line below necessary? I have commented it out
##age <- raster("")
ageVal <- getValues(age) #this will take a few seconds, big raster (4.6 Gb)
#age is now a vector 
#Most age values are 0 so remove them, then histogram
##Ian: we can't we remove all the 0, some stand will be at 0 post disturbance, so you are loosing forested pixels here. This age raster has the 623940100 pixels we are expecting (see item 2-). We need to keep all those pixels
## CONLCUSION: we need all the pixels inthe raster age1.tif

## DO NOT GET RID OF ALL THE 0s
# onlyTrees <- ageVal[ageVal != 0]
# hist(onlyTrees, breaks = 15)


#To group into 10 year age classes and put back into raster

## Ian: why do this? the increments are specific to the years on the growth curve and the model runs annually...reason for lumping them that I am missing? I think we can remove this
ageVal[ageVal != 0] <- round(onlyTrees/10, digits = 0)*10
ageClass <- setValues(age, ageVal)
plot(ageClass)
rm(ageClass, ageVal, age)
```
...
# what if I just get the values and compare the vectors? 
...
age1_values[age1_values == 0] <- NA # '1' was missing from subset in the script you PR to Celine
...
```{r}
nStands <- length(sim$ages) #I guess this isn't true because we will have grouped pixels into stands
...
```{r}
#IE: Here is my confusion....
#SpatialUnits
#I can't find any way to recreate the spatial unit IDs, species IDs, or forest type IDs from spadesCBMinputs/data/SK_ReclineRuns30m/LookupTables/yieldRCBM.csv
#I also don't understand where the productivity values are used in the gcID, or growthcurve table look up
#age
library(raster)
age1 <- raster('data/forIan/SK_data/rasters/age1.tif')
#
species <- raster("data/forIan/SK_data/rasters/casfri_dom2.tif") #This matches original age
speciesVal <- getValues(species)
...
spUnits_sub <- spTransform(spUnits_Can, CRSobj = age1@crs)
spUnits_sub <- crop(spUnits_sub, y = age1)

#Productivty
prod <- raster("data/forIan/SK_data/SK_ReclineRuns30m/layers/site_productivity_recliner.tif") #productivity for the small extent
#Clip to study area
