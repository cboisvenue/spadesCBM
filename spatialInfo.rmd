...
temp <- data.table(LeadingSp, Productivity)

gc <- data.table(gc) %>%
  .[,.("Productivity" = RasterValue,"LeadingSp" = rasterSps, "gcID" = growth_curve_component_id) ]

test <- left_join(temp, gc, by = c("Productivity" ,"LeadingSp"))


spUVal <- getValues(spUnits)
cohort <- data.table::data.table(spUVal, ldSpVal,prodVal)
cohort$PixelGroupID <- as.numeric(factor(paste(cohort$spUVal, cohort$ldSpVal, cohort$prodVal)))

testRas <- raster(spUnits)
testRas <- setValues(testRas, cohort$PixelGroupID)
plot(testRas)
