...
```{r}
#Crop to smal study area for now
species <- raster("data/forIan/SK_data/rasters/casfri_dom2.tif") #This matches original age
species <- crop(species, y = age_ras)

#Spatial Units, cropping to small area
spUnits_Can <- shapefile("data/forIan/SK_data/SK_ReclineRuns30m/layers/pspu.shp")
spUnits_Can <- spTransform(spUnits_Can, CRSobj = age_ras@crs)
spUnits_Can <- crop(spUnits_Can, y = age_ras)
#To convert to raster, first make empty raster with same resolution as age_ras
spUnits_ras <- raster(spUnits_Can, res = res(age_ras))
spUnits_ras <- rasterize(x = spUnits_Can, field = spUnits_Can$PSPU_ID, y = spUnits_ras)
plot(spUnits_ras)
#Delete this line it is just for Github Experimentation
#This is more Git Experimentation
```
