...
Age <- getValues(age)
temp <- data.table(LeadingSp, Productivity, SpUnit)

gc <- data.table(gc) %>%
  .[,.("Productivity" = RasterValue,"LeadingSp" = rasterSps, "gcID" = growth_curve_component_id, "SpUnit" = spatial_unit_id) ]

master <- left_join(temp, gc, by = c("Productivity" ,"LeadingSp", "SpUnit"))
master$Age <- Age
#Results in NAs where leading species == 0

forest <- data.table(master) %>%
  .[(is.na(gcID) == FALSE)]

forest$PixelGroupID <- as.numeric(factor(paste(forest$spUVal, forest$gcID, forest$Age)))
