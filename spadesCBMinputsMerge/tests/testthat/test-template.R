...
#    `functionName` is a function's name in your module (e.g., `spadesCBMinputsMergeEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Celine/github/spadesCBM/spadesCBMinputsMerge/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("spadesCBMinputsMerge")
  ...
  parameters <- list(
    ...
    spadesCBMinputsMerge = list(.saveInitialTime = NA)
  )
  ...
  if (exists("spadesCBMinputsMergeEvent1", envir = .GlobalEnv)) {
    simOutput <- spadesCBMinputsMergeEvent1(mySim)
  } else {
  ...
  if (exists("spadesCBMinputsMergeEvent2", envir = .GlobalEnv)) {
    simOutput <- spadesCBMinputsMergeEvent2(mySim)
  } else {
