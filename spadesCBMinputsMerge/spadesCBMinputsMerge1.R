...
defineModule(sim, list(
  ...
  version = list(SpaDES.core = "0.2.9", spadesCBMinputsMerge = "0.0.0.9000"),
  timeframe = as.POSIXlt(c(NA, NA)),
  timeunit = "year",
  citation = list("citation.bib"),
  documentation = deparse(list("README.txt", "spadesCBMinputsMerge.Rmd")),
  reqdPkgs = list(),
  parameters = rbind(
...
doEvent.spadesCBMinputsMerge = function(sim, eventTime, eventType) {
  switch(
    eventType,
    init = {
      ...
      sim <- scheduleEvent(sim, P(sim)$.plotInitialTime, "spadesCBMinputsMerge", "plot")
    ...
    plot = {
      ...
      # e.g.,
      #sim <- scheduleEvent(sim, time(sim) + P(sim)$.plotInterval, "spadesCBMinputsMerge", "plot")
    ...
    event1 = {
      ...
      # e.g.,
      # sim <- scheduleEvent(sim, time(sim) + increment, "spadesCBMinputsMerge", "templateEvent")
