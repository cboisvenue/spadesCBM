...
defineModule(sim, list(
  ...
  parameters = rbind(
    ...
    defineParameter(
      "spinupDebug", "logical", FALSE, NA, NA,
      paste(
        "If TRUE spinupResult will be outputed to a text file (spinup.csv).",
        "FALSE means no output of the spinupResult."
      )
    ),
    defineParameter(
      "noAnnualDisturbances", "logical", FALSE, NA, NA,
      paste(
        "If TRUE the sim$allProcesses and sim$opMatrix are created in the postSpinup event, just once.",
        "By default, these are recreated everyyear in the annual event"
      )
    ),
    defineParameter(
      "poolsToPlot", "character", "totalCarbon", NA, NA,
      "which carbon pools to plot, if any. Defaults to total carbon"
    ),
    defineParameter(
  ...
  outputObjects = bind_rows(
    ...
    createsOutput(
      ...
      desc = "this is modified: ordered by pixelGroup"
    ),
    createsOutput(
      objectName = "nStands", objectClass = "integer",
      desc = "number of pixelGroup in this annual run"
    ),
    createsOutput(
      objectName = "gcids", objectClass = "vector",
      desc = "growth component id associated with each pixelGroup"
    ),
    createsOutput(
      objectName = "spatialUnits", objectClass = "vector",
      desc = "spatial unit for each pixelGroup"
    ),
    createsOutput(
      objectName = "ecozones", objectClass = "vector",
      desc = "ecozone for each pixelGroup"
    ),
    createsOutput(
      objectName = "turnoverRates", objectClass = "data.table",
...
annual <- function(sim) {
  ...
  distPixels <- sim$spatialDT[events > 0, .(
    pixelIndex, pixelGroup, ages, rasterSps, Productivity,
  ...
  groupToAddC <- sim$pixelGroupC[
    which(sim$pixelGroupC$pixelGroup %in% unique(distPixels$pixelGroup)),
  ...
  distPixels <- distPixels[, .(
    pixelIndex, ages, rasterSps, Productivity, spatial_unit_id,
    growth_curve_component_id, growth_curve_id, events, oldGroup, newGroup
  )]
  ...
  newGroups <- distPixels[, .(pixelIndex, newGroup)] %>%
    .[, pixelGroup := newGroup] %>%
    .[, newGroup := NULL]
...
##### "assertions": verify in R what is being done in C++ ------------------------------------------
  ...
  addDistC <- cbind(
    toAdd[, .(ages, rasterSps, spatial_unit_id, growth_curve_component_id, growth_curve_id, Productivity)],
    toAddOut
  )
  ...
  updatePools <- cbind(
    rep(time(sim)[1], length(sim$ages)), pixelCount[1:length(sim$ages), 2],
    pixelGroupForAnnual$pixelGroup, sim$ages, sim$pools
  )
...
.inputObjects <- function(sim) {
  ...
  if (!suppliedElsewhere(sim$historicDMIDs)) {
    sim$historicDMIDs <- c(214)
  } # ,1,1,1)
  if (!suppliedElsewhere(sim$lastPassDMIDS)) {
    sim$lastPassDMIDS <- c(214)
  } # ,1,1,1)
  ...
  # if (!suppliedElsewhere(sim$disturbanceEvents)) {
  ...
  if (!suppliedElsewhere(sim$disturbanceRasters)) {
    sim$disturbanceRasters <- list.files("data/forIan/SK_data/CBM_GIS/disturbance_testArea", # TODO: use module data dir
      full.names = TRUE
    ) %>%
      grep(., pattern = "[.]grd$", value = TRUE)
  }
  if (!suppliedElsewhere(sim$spatialDT)) {
    age <- raster(file.path("data/forIan/SK_data/CBM_GIS/age_TestArea.tif")) # TODO: use module data dir
    # This works
    ages <- getValues(age)
    # read-in species
