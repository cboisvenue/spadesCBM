...
These spatial units are the link to ALL the default parameters to run CBM across Canada.
...
age <- raster(file.path("data/forIan/SK_data/rasters/age1.tif"))
...
```{r compare Age, echo=FALSE, include=FALSE}
library(raster)
# Firstcrop to a smaller area first for faster comparisons
small_extent <- raster(file.path("data/forIan/SK_data/SK_ReclineRuns30m/layers/age1_recliner.tif")) #This is smaller
...
```{r age vector, echo=FALSE, include=FALSE}
library(raster)
...
library(raster)
##age
age <- raster('data/forIan/SK_data/CBM_GIS/age_TestArea.tif')
...
#load gcID look up
gc <- read.csv("data/forIan/SK_data/gcID_ref.csv")
simgcCompID <- read.csv("spadesCBMinputs/data/SK_ReclineRuns30m/LookupTables/yieldComponentRCBM.csv") #enter age and gcID

#reclassify prod and species to gcID
library(data.table)
library(magrittr)
library(dplyr)

LeadingSp <- getValues(species)
Productivity <- getValues(prod)
SpUnit <- getValues(spUnits)
Age <- getValues(age)
temp <- data.table(LeadingSp, Productivity, SpUnit, Age)

gc <- data.table(gc) %>%
  .[,.("Productivity" = RasterValue,"LeadingSp" = rasterSps, growth_curve_component_id, "SpUnit" = spatial_unit_id) ]

master <- as.data.table(left_join(temp, gc, by = c("Productivity" ,"LeadingSp", "SpUnit")))
#MERGE IS ORDERING BUT THEN WE LOSE SPATIAL LOCATIONS)
#Results in NAs where leading species == 0
forest <- master[(is.na(growth_curve_component_id) == FALSE)]
forest$PixelGroupID <- as.numeric(factor(paste(forest$spUVal, forest$growth_curve_component_id, forest$Age)))

testRas <- raster(spUnits)
testRas[LeadingSp == 0] <- NA
testRas[LeadingSp != 0] <- forest$PixelGroupID
plot(testRas)
forest$PixelGroupID
#
...
This will come from the Wulder and White work.
There is a list for SK here: `SK_ReclineRuns30m\layers\projected_change_type`

***TASKS***  
1. get the prototype working with one disturbance  ***DONE***
...
spatial_unit_id <- as.integer(getValues(spuRaster)) #28 27
# make it a data.table
level2DT <- as.data.table(cbind("LeadingSp" = rasterSps,"Productivity" = RasterValue, spatial_unit_id, "Age" = ages))

#Join to Growth Curves
# add the gcID
...
#filter the spatial data by unique values
...
level3DT$PixelGroupID <- as.numeric(factor(paste(level3DT$spatial_unit_id, level3DT$growth_curve_component_id, level3DT$Age)))

# all unique combinations
# dim(unique(level3DT))
# [1] 759   8

#This sort = FALSE is crucial to preserving spatial data
spatialDT <- merge(level2DT, gcID, by = c("LeadingSp", "Productivity", "spatial_unit_id"), sort = FALSE)

#This was Eliot's suggestion as an alternative to the LBMR method
spatialDT$PixelGroupID <- as.numeric(as.factor(paste(spatialDT$spatial_unit_id, spatialDT$growth_curve_component_id, spatialDT$Age)))

#to set back into raster, need Raster (record of what is NA), at least first time. 
masterRaster <- raster(spuRaster)
masterRaster[rasterSps != 0] <- spatialDT$PixelGroupID[!is.na(spatialDT$growth_curve_component_id)] #1347529 pixels
plot(masterRaster)


#example linking disturbance
# fire1987 <- raster("data/forIan/SK_data/CBM_GIS/disturbance/1987.tif")
# plot(fire1987) #not sure what these values mean, range = 5000-25000. Ask Byron? Anyway lets pretend all pixels are burned


#We probably do not want to generate one raster for every pool. Maybe make function that does these steps?
poolsDT <- as.data.table(spadesCBMout$cbmPools)
poolsDT <- poolsDT[, .("PixelGroupID" = standindex, BelowGroundSlowSoil, "new.age" = age)] #not sure if we want to join all....
#The first subset calculates the max age, the second subset selects only rows where age == max.age
poolsDT <- poolsDT[, max.age := max(new.age), by = .(PixelGroupID)][new.age == max.age, .(BelowGroundSlowSoil, PixelGroupID, new.age)]

#Each stand index has ten values. Need to get the max of each and subset by that. 
masterTable <- right_join(poolsDT, spatialDT, by = "PixelGroupID", sort = FALSE)  #Need to make this merge

outputBelowGroundSlowSoil <- masterRaster
outputBelowGroundSlowSoil[!is.na(outputBelowGroundSlowSoil)] <- masterTable$BelowGroundSlowSoil

plot(outputBelowGroundSlowSoil)
########################### DATA.TABLE COMPLETE #########################################
```