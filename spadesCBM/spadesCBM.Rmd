...
spadesCBMSimRIA <- simInit(times = times,
                        params = parameters,
                        modules = modules, 
                        objects = objects, 
                        paths = paths)
dev()
clearPlot()
spadesCBMRIA <- spades(spadesCBMSimRIA,debug=TRUE)
...
The user provides growth curves in m3/ha of cumulative m3/ha over time. Those curves are fed into the Boudewyn algorithms (spadesCBMinputs module) with its results multiplied by 0.5 to give carbon. Boudewyn gives us the cumBiom, line 240 in spadesCBMinputs in the SK runs, that is the cumulative biomass for the three above-ground live pools in tonnes of carbon/ha. The simgrowth_increments are then calculated by the difference between two years on the cumBiom*0.5 curve. These are in tonnes of carbon per ha and these MATCH the values if you calculate the differences between two years in the cPoolsYears.csv (output of $cbmPools, assuming no overmature decline). Root increments are calculated later and the details of that are in Matrices.xlm and workingOutNPP.xlsx. Average NPP is 2.5987 for SK run and this is in tonnes of C per hectare per year.
