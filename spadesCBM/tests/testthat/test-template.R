...
#    `functionName` is a function's name in your module (e.g., `spadesCBMEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Celine/GitHub/spadesCBM/spadesCBM/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("spadesCBM")
  ...
  parameters <- list(
    ...
    spadesCBM = list(.saveInitialTime = NA)
  )
  ...
  if (exists("spadesCBMEvent1", envir = .GlobalEnv)) {
    simOutput <- spadesCBMEvent1(mySim)
  } else {
  ...
  if (exists("spadesCBMEvent2", envir = .GlobalEnv)) {
    simOutput <- spadesCBMEvent2(mySim)
  } else {
