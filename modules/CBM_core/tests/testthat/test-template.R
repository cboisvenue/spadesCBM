...
#    `functionName` is a function's name in your module (e.g., `spadesCBMcoreEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Celine/GitHub/spadesCBM/spadesCBMcore/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("spadesCBMcore")
  ...
  parameters <- list(
    ...
    spadesCBMcore = list(.saveInitialTime = NA)
  )
  ...
  if (exists("spadesCBMcoreEvent1", envir = .GlobalEnv)) {
    simOutput <- spadesCBMcoreEvent1(mySim)
  } else {
  ...
  if (exists("spadesCBMcoreEvent2", envir = .GlobalEnv)) {
    simOutput <- spadesCBMcoreEvent2(mySim)
  } else {
