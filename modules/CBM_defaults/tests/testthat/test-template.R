...
#    `functionName` is a function's name in your module (e.g., `spadesCBMdefaultsEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Celine/GitHub/spadesCBM/spadesCBMdefaults/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("spadesCBMdefaults")
  ...
  parameters <- list(
    ...
    spadesCBMdefaults = list(.saveInitialTime = NA)
  )
  ...
  if (exists("spadesCBMdefaultsEvent1", envir = .GlobalEnv)) {
    simOutput <- spadesCBMdefaultsEvent1(mySim)
  } else {
  ...
  if (exists("spadesCBMdefaultsEvent2", envir = .GlobalEnv)) {
    simOutput <- spadesCBMdefaultsEvent2(mySim)
  } else {
