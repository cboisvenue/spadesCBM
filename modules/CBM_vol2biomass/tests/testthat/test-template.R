...
#    `functionName` is a function's name in your module (e.g., `spadesCBMm3ToBiomassEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Celine/github/spadesCBM/spadesCBMm3ToBiomass/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("spadesCBMm3ToBiomass")
  ...
  parameters <- list(
    ...
    spadesCBMm3ToBiomass = list(.saveInitialTime = NA)
  )
  ...
  if (exists("spadesCBMm3ToBiomassEvent1", envir = .GlobalEnv)) {
    simOutput <- spadesCBMm3ToBiomassEvent1(mySim)
  } else {
  ...
  if (exists("spadesCBMm3ToBiomassEvent2", envir = .GlobalEnv)) {
    simOutput <- spadesCBMm3ToBiomassEvent2(mySim)
  } else {
